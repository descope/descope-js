...
describe('user-management-widget notification escaping', () => {
  ...
  // The count-based ones (deleteUsers) return a fixed "Failed to delete user"
  // and never include the error text, by design.
  it('keeps the error text readable and the wrapper markup intact', () => {
    const [content] = rejectAndParse(createUser as unknown as ActionEntry);
    ...
    const title = Array.from(content.querySelectorAll('div')).find(
      (ele) => ele.textContent?.trim().startsWith('Failed to create user'),
    );
