...
const generateSdkResponse = ({
	...
	data: {
		...
		status,
		authInfo: 'auth info',
		webauthn: { options: webAuthnOptions, transactionId: webAuthnTransactionId },
		error
	},
	error: {
...
describe('debugger', () => {
	...
	it('should render the debugger with an empty state when debug is set to true', async () => {
		...
		await waitFor(() =>
			expect(screen.getByShadowText('No errors detected 👀')).toBeInTheDocument()
		);
	});
	...
	it('should add a debugger message when got a response error', async () => {
		...
		await waitFor(() => expect(screen.getByShadowText('error message!')).toBeInTheDocument());
	...
	it('should add a debugger message when receiving a flow response error', async () => {
		...
		await waitFor(() => expect(screen.getByShadowText('[123]: description')).toBeInTheDocument());
		await waitFor(() => expect(screen.getByShadowText('message')).toBeInTheDocument());
	});
	...
	it('should remain visible when resizing the window', async () => {
		...
		await waitFor(() => expect(screen.getByShadowText('No errors detected 👀')).toBeVisible());
	});
	...
	it('should toggle debugger when flag changes', async () => {
		...
		await waitFor(() => expect(document.getElementsByTagName('descope-debugger').length).toBe(0));
	});
	...
	it('should collapse message when clicking on its title', async () => {
		...
		expect(screen.getByShadowText('error message!').parentElement.parentElement).toHaveClass(
			'collapsed'
		);
	});
});
