...
jest.mock('../../src/lib/helpers/helpers', () => ({ withMemCache: (fn) => fn }));
...
describe('WebAuthn Helper Function', () => {
	...
	beforeAll(() => {
		...
		Object.defineProperty(global, 'AuthenticatorAttestationResponse', { value: TestClass });
	...
	describe('isConditionalLoginSupported', () => {
		...
		it('should return the correct value when browser supports webauthn', async () => {
			(<any>window.PublicKeyCredential).isConditionalMediationAvailable = jest.fn(() => true);
			(<any>window.PublicKeyCredential).isUserVerifyingPlatformAuthenticatorAvailable = jest.fn(
				() => true
			);
		...
		it('should not throw when browser function rejects', async () => {
			(<any>window.PublicKeyCredential).isConditionalMediationAvailable = jest.fn(async () => {
				throw Error();
			});
			(<any>window.PublicKeyCredential).isUserVerifyingPlatformAuthenticatorAvailable = jest.fn(
				() => true
