...
const isConditionalLoginSupportedMock = isConditionalLoginSupported as jest.Mock;
...
describe('webauthnConditionalUi', () => {
	...
	it('should change the webauthn input name and add a "user-" prefix on load', async () => {
		...
		await waitFor(() =>
			expect(screen.getByShadowPlaceholderText('test')).toHaveAttribute('name', 'user-test')
		);
	});
	...
	it('should not throw when webauthn start call fails', async () => {
		...
		await waitFor(() =>
			expect(errorSpy).toHaveBeenCalledWith('Webauthn start failed', '', expect.any(Error))
		);
	});
	...
	it('should add form element when browser is Chrome', async () => {
		...
		Object.defineProperty(navigator, 'userAgent', { value: 'Chrome', writable: true });
		...
		Object.defineProperty(navigator, 'userAgent', { value: origUserAgent, writable: true });
	...
	it('should not add form element when browser is different from Chrome', async () => {
		...
		Object.defineProperty(navigator, 'userAgent', { value: 'Test', writable: true });
		Object.defineProperty(navigator, 'vendor', { value: 'Google Inc', writable: true });

		document.body.innerHTML = `<h1>Custom element test</h1> <descope-wc flow-id="otpSignInEmail" project-id="1"></descope-wc>`;

		await waitFor(() => expect(document.querySelector('form')).not.toBeInTheDocument());
	...
	it('should not add form element when WC is wrapped with a form element', async () => {
		...
		await waitFor(() => expect(document.querySelectorAll('form').length).toBe(1));
	});
});
