...
describe('autoRefresh', () => {
  ...
  describe('customActivityTracking mode', () => {
    ...
    it('should skip refresh when user is idle when timer fires and nextRefreshSeconds is present', async () => {
      ...
      // Timer fires but user has been idle (activity was reset when session was received)
      timeoutFn();

      expect(refreshSpy).not.toHaveBeenCalled();
      expect(loggerDebugMock).toHaveBeenCalledWith(
        'Skipping refresh due to timer - user is idle',
      );
    });
    ...
    it('should trigger refresh when markUserActive is called after idle skip', async () => {
      ...
      // Timer fires - user is idle, refresh skipped
      timeoutFn();
      expect(refreshSpy).not.toHaveBeenCalled();
    ...
    it('should proceed with refresh when timer fires after user marked active', async () => {
      const setTimeoutSpy = jest.spyOn(global, 'setTimeout');
  ...
  it('should not refresh on visibilitychange when a refresh is already in flight', async () => {
    const setTimeoutSpy = jest.spyOn(global, 'setTimeout');
    ...
    // the timer fires while the tab is visible - a refresh starts and stays in flight
    Object.defineProperty(document, 'visibilityState', {
      value: 'visible',
      writable: true,
      configurable: true,
    ...
    // the tab is hidden and shown again within the refresh threshold - no second refresh
    jest.useFakeTimers();
    try {
      jest.setSystemTime(Date.now() + 30 * 1000); // 15 seconds before expiration
      document.dispatchEvent(new Event('visibilitychange'));
  ...
  it('should not refresh on visibilitychange when user is idle and inactivity timeout is configured', async () => {
    const loggerDebugMock = logger.debug as jest.Mock;
    ...
    jest.useFakeTimers();
    try {
      // the user returns close to expiration but has been idle - the session is not extended
      jest.setSystemTime(Date.now() + 30 * 1000); // 15 seconds before expiration
      Object.defineProperty(document, 'visibilityState', {
        value: 'visible',
        writable: true,
      ...
      expect(refreshSpy).not.toHaveBeenCalled();
      expect(loggerDebugMock).toHaveBeenCalledWith(
        'Skipping refresh due to visibility - user is idle',
      );

      // the user becomes active - the skipped refresh is caught up
      (sdk as any).markUserActive();
      expect(refreshSpy).toHaveBeenCalledWith(authInfo.refreshJwt);
    } finally {
      jest.useRealTimers();
