...
| send-session-token                        | **"true"** - Send the current session JWT on flow start/next requests, exposing its validated claims to the flow via the `sessionJwtClaims` context key</br>**"false"** - Do not send the session JWT</br>                                 | **"false"**   |
...
When you use `createSdk` from `@descope/web-js-sdk`, the `onSessionTokenChange` callback will **not** fire automatically after flows complete inside `<descope-wc>`.
