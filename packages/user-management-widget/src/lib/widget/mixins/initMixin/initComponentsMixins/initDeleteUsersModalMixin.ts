...
import { getSelectedUsersDetailsForDisplay, getSelectedUsersUserIds } from '../../../state/selectors';
import { stateManagementMixin } from '../../stateManagementMixin';
import { initWidgetRootMixin } from './initWidgetRootMixin';

export const initDeleteUsersModalMixin = createSingletonMixin(<T extends CustomElementConstructor>(superclass: T) =>
  class InitDeleteUsersModalMixinClass extends compose(stateManagementMixin, modalMixin, loggerMixin, initWidgetRootMixin)(superclass) {
    ...
    async #initDeleteUserModal() {
      ...
      const cancelButton = new ButtonDriver(() => this.deleteUsersModal.ele?.querySelector('[data-id="modal-cancel"]'), { logger: this.logger });
      cancelButton.onClick(() => this.deleteUsersModal.close());

      const submitButton = new ButtonDriver(() => this.deleteUsersModal.ele?.querySelector('[data-id="modal-submit"]'), { logger: this.logger });
      ...
      const description = new TextDriver(this.deleteUsersModal.ele?.querySelector('[data-id="body-text"]'), { logger: this.logger });
