...
type Tail<T extends ReadonlyArray<string>> = T extends readonly [head: any, ...tail: infer Tail_]
	? Tail_
	: never;
...
type SdkFnWrapperInternal<F extends SdkFn<ResponseData>, R extends ResponseData> = (
	fn: (...args: Parameters<F>) => ReturnType<F>
) => (
	...args: Parameters<F>
) => Promise<
	SdkResponse<R extends Record<string, never> ? SdkResponseType<F> : SdkResponseType<F> & R>
>;
...
> = Head<Paths> extends never // if there are no paths on the list
	...
	? ReplacePaths<ReplacePath<Obj, Head<Paths>, WrapperData>, Tail<Paths>, WrapperData> // recursive call with the updated object of all the previous paths, and the remaining paths
...
> = Path extends `${infer Head}.${infer Tail}`
	? { [Key in keyof Obj]: Key extends Head ? ReplacePath<Obj[Key], Tail, WrapperData> : Obj[Key] } // if it's not the leaf key, recursive call with the nested path
	: {
...
export type SdkFn<T extends ResponseData> = (...args: any) => Promise<SdkResponse<T>>;

// should be used to type the wrapper functions
export type SdkFnWrapper<Z extends ResponseData> = <A extends any[], R extends ResponseData>(
	fn: (...args: A) => Promise<SdkResponse<R>>
) => (...args: A) => Promise<SdkResponse<Z extends Record<string, never> ? R : Z & R>>;
