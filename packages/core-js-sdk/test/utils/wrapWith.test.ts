...
describe('wrapWith', () => {
	...
	it('should throw an error when path does not exist', () => {
		...
		expect(() => wrapWith(obj, ['b.fn'] as unknown as ['a.fn'], wrapper)).toThrow(
			'Invalid path "b.fn", "b" is missing or has no value'
		);
	});
	...
	it('should throw an error when path is not a fn', () => {
		...
		expect(() => wrapWith(obj, ['a.f1'] as unknown as ['a.fn'], wrapper)).toThrow(
			'"a.f1" is not a function'
		);
	});
});
