...
describe('httpClient', () => {
	it('should call fetch with the correct params when calling "get"', () => {
		httpClient.get('1/2/3', { headers: { test2: '123' }, queryParams: { test2: '123' } });

		expect(mockFetch).toHaveBeenCalledWith(new URL('http://descope.com/1/2/3?test2=123'), {
			body: undefined,
			credentials: 'include',
			headers: new Headers({
				test2: '123',
	...
	it('should call the "afterHook"', async () => {
		...
		expect(afterRequestHook).toHaveBeenCalledWith(expect.objectContaining({ path: '1/2/3' }), {
			key: 'val'
		});
	});
	...
	it('should call fetch with project id in bearer token when null is passed as a token', () => {
		...
		expect(mockFetch).toHaveBeenCalledWith(new URL('http://descope.com/1/2/3'), {
			body: undefined,
			credentials: 'include',
			headers: new Headers({
				Authorization: 'Bearer 456',
	...
	it('should not throw when not providing config or logger', () => {
		expect(createHttpClient({ baseUrl: 'http://descope.com', projectId: '456' }).get).not.toThrow();
	});
});
