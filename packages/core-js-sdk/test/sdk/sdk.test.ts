...
describe('sdk', () => {
	...
	describe('getJwtPermissions', () => {
		...
		it('should return empty permissions for tenant', () => {
			(jwtDecode as jest.Mock).mockImplementation(() => mock);
			expect(sdk.getJwtPermissions('jwt', 'C2EdY4UXXzKPV0EKdZFJbuKKmvtl')).toStrictEqual([]);
		});
		it('should return two roles for tenant', () => {
			(jwtDecode as jest.Mock).mockImplementation(() => mock);
			expect(sdk.getJwtRoles('jwt', 'C2EdY4UXXzKPV0EKdZFJbuKKmvtl')).toStrictEqual(['abc', 'xyz']);
		});
	});
	...
	describe('logout', () => {
		it('should send the correct request', () => {
			...
			sdk.logout('token');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.logout, {}, { token: 'token' });
		});
	});
	...
	describe('logoutAll', () => {
		it('should send the correct request', () => {
			...
			sdk.logoutAll('token');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.logoutAll, {}, { token: 'token' });
		});
	});
	...
	describe('me', () => {
		it('should send the correct request', () => {
			...
			sdk.me('token');
			expect(mockHttpClient.get).toHaveBeenCalledWith(apiPaths.me, { token: 'token' });
		});
	});
});
