...
describe('totp', () => {
	...
	describe('verify', () => {
		it('should throw an error when loginId is not a string', () => {
			expect(() => sdk.totp.verify(1, '123456')).toThrow('"loginId" must be a string');
		});

		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.totp.verify('', '123456')).toThrow('"loginId" must not be empty');
		});

		it('should throw an error when code is not a string', () => {
			expect(() => sdk.totp.verify('loginId', 1)).toThrow('"code" must be a string');
		});

		it('should throw an error when code is empty', () => {
			expect(() => sdk.totp.verify('loginId', '')).toThrow('"code" must not be empty');
		});
		...
		it('should send the correct request with login options', () => {
			sdk.totp.verify('loginId', '123456', { stepup: true, customClaims: { k1: 'v1' } }, 'token');
			expect(mockHttpClient.post).toHaveBeenCalledWith(
				apiPaths.totp.verify,
				{
					code: '123456',
	...
	describe('update', () => {
		it('should throw an error when loginId is not a string', () => {
			expect(() => sdk.totp.update(1, '123456')).toThrow('"loginId" must be a string');
		});

		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.totp.update('', '123456')).toThrow('"loginId" must not be empty');
		});
