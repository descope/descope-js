...
describe('Magic Link', () => {
	...
	describe('signUp', () => {
		it('should throw an error when loginId is not a string', () => {
			expect(() => sdk.magicLink.signUp.email(undefined)).toThrow('"loginId" must be a string');
		});

		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.magicLink.signUp.email('')).toThrow('"loginId" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.magicLink.signUp.email('loginId', 'http://some.thing.com', { name: 'John Doe' });
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.magicLink.signUp + '/email', {
				loginId: 'loginId',
	...
	describe('signUpOrIn', () => {
		it('should throw an error when loginId is not a string', () => {
			expect(() => sdk.magicLink.signUpOrIn.email(undefined)).toThrow('"loginId" must be a string');
		});

		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.magicLink.signUpOrIn.email('')).toThrow('"loginId" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.magicLink.signUpOrIn.email('loginId', 'http://some.thing.com');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.magicLink.signUpOrIn + '/email', {
				loginId: 'loginId',
				URI: 'http://some.thing.com'
			});
		});
	...
	describe('verify', () => {
		...
		it('should throw an error when token is empty', () => {
			expect(() => sdk.magicLink.verify('')).toThrow('"token" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.magicLink.verify('token');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.magicLink.verify, {
				token: 'token'
			});
		});
	});
	...
	describe('update', () => {
		describe('email', () => {
			it('should throw an error when loginId is not a string', () => {
				expect(() => sdk.magicLink.update.email(1, '123456')).toThrow('"loginId" must be a string');
			});
			...
			it('should throw an error when email is not a string', () => {
				expect(() => sdk.magicLink.update.email('loginId', 1)).toThrow('"email" must be a string');
			});
			...
			it('should send the correct request', () => {
				...
				sdk.magicLink.update.email('loginId', 'new@email.com', 'new@email.com', 'token');
				expect(mockHttpClient.post).toHaveBeenCalledWith(
			...
			it('should return the correct response', async () => {
				...
				mockHttpClient.post.mockResolvedValue(httpResponse);
				const resp = await sdk.magicLink.update.email('loginId', 'new@email.com', 'new@email.com');
		...
		describe('phone', () => {
			...
			it('should throw an error when phone is not in phone format', () => {
				expect(() => sdk.magicLink.update.phone.sms('loginId', 'nonPhone')).toThrow(
			...
			it('should send the correct request', () => {
				...
				mockHttpClient.post.mockResolvedValue(httpResponse);
				sdk.magicLink.update.phone.sms('loginId', '+9720000000', 'http://some.thing.com', 'token');
				expect(mockHttpClient.post).toHaveBeenCalledWith(
