...
describe('Flows', () => {
	...
	describe('start', () => {
		...
		it('should return the correct response', async () => {
			const httpRespJson = { flowId: 'f1', taskId: 't1', status: 'completed', screenId: 'screen1' };
			const httpResponse = {
				ok: true,
				json: () => httpRespJson,
				clone: () => ({
	...
	describe('next', () => {
		it('should throw an error when executionId is not a string', () => {
			expect(() => sdk.flow.next(undefined, 's1', 'a1')).toThrow('"executionId" must be a string');
		});

		it('should throw an error when executionId is empty', () => {
			expect(() => sdk.flow.next('', 's1', 'a1')).toThrow('"executionId" must not be empty');
		});

		it('should throw an error when stepId is not a string', () => {
			expect(() => sdk.flow.next('f1', undefined, 'a1')).toThrow('"stepId" must be a string');
		});

		it('should throw an error when stepId is empty', () => {
			expect(() => sdk.flow.next('f1', '', 'a1')).toThrow('"stepId" must not be empty');
		});
		...
		it('should throw an error when interactionId is empty', () => {
			expect(() => sdk.flow.next('f1', 's1', '')).toThrow('"interactionId" must not be empty');
		});
