...
describe('otp', () => {
	...
	describe('signUp', () => {
		...
		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.otp.signUp.email('')).toThrow('"loginId" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.otp.signUp.email('loginId', { name: 'John Doe' });
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.otp.signUp + '/email', {
				loginId: 'loginId',
				user: { name: 'John Doe' }
			});
		});
	...
	describe('signIn', () => {
		...
		it('should send the correct request with login options', () => {
			sdk.otp.signIn.email('loginId', { stepup: true, customClaims: { k1: 'v1' } }, 'token');
			expect(mockHttpClient.post).toHaveBeenCalledWith(
				apiPaths.otp.signIn + '/email',
				{
					loginId: 'loginId',
	...
	describe('signUpOrIn', () => {
		...
		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.otp.signUpOrIn.email('')).toThrow('"loginId" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.otp.signUpOrIn.email('loginId');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.otp.signUpOrIn + '/email', {
				loginId: 'loginId'
			});
		});
	...
	describe('verify', () => {
		it('should throw an error when loginId is not a string', () => {
			expect(() => sdk.otp.verify.email(1, '123456')).toThrow('"loginId" must be a string');
		});

		it('should throw an error when loginId is empty', () => {
			expect(() => sdk.otp.verify.email('', '123456')).toThrow('"loginId" must not be empty');
		});

		it('should throw an error when code is not a string', () => {
			expect(() => sdk.otp.verify.email('loginId', 1)).toThrow('"code" must be a string');
		});

		it('should throw an error when code is empty', () => {
			expect(() => sdk.otp.verify.email('loginId', '')).toThrow('"code" must not be empty');
		});

		it('should send the correct request', () => {
			sdk.otp.verify.email('loginId', '123456');
			expect(mockHttpClient.post).toHaveBeenCalledWith(apiPaths.otp.verify + '/email', {
				code: '123456',
				loginId: 'loginId'
			});
		});
	...
	describe('update', () => {
		describe('email', () => {
			it('should throw an error when loginId is not a string', () => {
				expect(() => sdk.otp.update.email(1, '123456')).toThrow('"loginId" must be a string');
			});

			it('should throw an error when loginId is empty', () => {
				expect(() => sdk.otp.update.email('', '123456')).toThrow('"loginId" must not be empty');
			});

			it('should throw an error when email is not a string', () => {
				expect(() => sdk.otp.update.email('loginId', 1)).toThrow('"email" must be a string');
			});
		...
		describe('phone', () => {
			it('should throw an error when loginId is not a string', () => {
				expect(() => sdk.otp.update.phone.sms(1, '123456')).toThrow('"loginId" must be a string');
			});

			it('should throw an error when loginId is empty', () => {
				expect(() => sdk.otp.update.phone.sms('', '123456')).toThrow('"loginId" must not be empty');
			});

			it('should throw an error when email is not a string', () => {
				expect(() => sdk.otp.update.phone.sms('loginId', 1)).toThrow('"phone" must be a string');
			});
			...
			it('should send the correct request', () => {
				...
				mockHttpClient.post.mockResolvedValue(httpResponse);
				sdk.otp.update.phone.sms('loginId', '+9720000000', 'token');
				expect(mockHttpClient.post).toHaveBeenCalledWith(
