...
describe('sdk', () => {
	...
	it('should throw an error when projectId is missing', () => {
		expect(() => createSdk({ projectId: '' })).toThrow('"projectId" must not be empty');
	});
	...
	it('should override the default base url if provided', () => {
		...
		expect(createSdk({ projectId: '123', baseUrl: 'http://new.base.url' })).toBe('sdk');
		expect(sdk).toHaveBeenCalledWith('httpClient');
		expect(httpClient).toHaveBeenCalledWith(
	...
	it('should add base headers if provided', () => {
		...
		expect(createSdk({ projectId: '123', baseHeaders: { header: '123' } })).toBe('sdk');
		expect(sdk).toHaveBeenCalledWith('httpClient');
		expect(httpClient).toHaveBeenCalledWith(
			expect.objectContaining({
