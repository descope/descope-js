...
describe('lastLoggedInUser', () => {
	...
	it('should send last user in start option if set', async () => {
		...
		const mockFetch = jest.fn().mockReturnValue(createMockReturnValue(flowResponse));
		global.fetch = mockFetch;
		const sdk = createSdk({ projectId: 'pid' });
		await sdk.flow.start('id', { tenant: 'yo' });
		expect(mockFetch).toBeCalledWith(
			expect.objectContaining({ href: 'https://api.descope.com/v1/flow/start' }),
			expect.any(Object)
		);
	...
	it('should set local storage on completed start response', async () => {
		const mockFetch = jest.fn().mockReturnValue(createMockReturnValue(completedFlowResponse));
		global.fetch = mockFetch;
		const sdk = createSdk({ projectId: 'pid' });
	...
	it('should set local storage on completed next response', async () => {
		const mockReturnVal = Promise.resolve(createMockReturnValue(completedFlowResponse));
	...
	it('should remove last user data on logout', async () => {
		...
		expect(localStorage.getItem(LOCAL_STORAGE_LAST_USER_DISPLAY_NAME)).toBeFalsy();
	});
});
