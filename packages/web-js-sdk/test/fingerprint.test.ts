...
import { authInfo, completedFlowResponse, flowResponse, mockFingerprint } from './mocks';
...
describe('fingerprint', () => {
	it('beforeRequest - should add visitor request and session id to outgoing requests', () => {
		...
		expect(mockFetch).toHaveBeenCalledWith(new URL(`https://api.descope.com/1/2/3?test2=123`), {
			body: undefined,
			headers: new Headers({
				test2: '123',
