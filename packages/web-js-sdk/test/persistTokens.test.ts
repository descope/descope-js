...
describe('persistTokens', () => {
	...
	it('should get refresh token from local storage', () => {
		...
		expect(mockFetch).toHaveBeenCalledWith(new URL(`https://api.descope.com/1/2/3`), {
			body: undefined,
			headers: new Headers({
	...
	it('should set session token as cookie and refresh token to local storage when managing session token via cookie', async () => {
		const mockFetch = jest.fn().mockReturnValue(createMockReturnValue(authInfo));
		global.fetch = mockFetch;

		const setMock = jest.spyOn(Cookies, 'set');

		const sdk = createSdk({ projectId: 'pid', sessionTokenViaCookie: true, persistTokens: true });
	...
	it('should not set refresh if persistTokens is configured to false', async () => {
		...
		const sdk = createSdk({ projectId: 'pid', autoRefresh: false, persistTokens: true });
		const refreshSpy = jest.spyOn(sdk, 'refresh').mockReturnValue(new Promise(() => {}));
		await sdk.httpClient.get('1/2/3');
